...
__global__ void gpu_initialization(double*, double*, double*, double*, double*, double*, double*, double*, double*, double*);
//__global__ void gpu_taylor_green(unsigned int,double*,double*,double*);
__global__ void gpu_init_equilibrium(double*,double*,double*,double*,double*, double*, double*, double*, double*,
...
__host__ void initialization(double *r, double *c, double *fi, double *u, double *v, double *w, double *ex, double *ey, double *ez, double *temp)
{
	// blocks in grid
	...
	gpu_initialization << <grid, threads >> > (r, c, fi, u, v, w, ex, ey, ez,temp);
	getLastCudaError("gpu_taylor_green kernel error");
}

__global__ void gpu_initialization(double *r, double *c, double *fi, double *u, double *v, double *w, double *ex, double *ey, double *ez,double *temp)
{
								...
								double *u, double *v, double *w, double *ex, double *ey, double *ez, double *temp)
{
    ...
    gpu_init_equilibrium<<< grid, threads >>>(f0,f1,h0,h1,temp0,temp1,r,c,u,v,w,ex,ey,ez,temp);
...
__host__ void stream_collide_save(double *f0, double *f1, double *f2, double *h0, double *h1, double *h2, double *temp0, double *temp1, double *temp2,
	double *r, double *c, double *u, double *v, double *w, double *ex, double *ey, double *ez, double *Temp, double t,double *f0bc)
{
    // blocks in grid
    dim3  grid(NX/nThreads, NY, NZ);
    // threads in block
    ...
    gpu_collide_save<<< grid, threads >>>(f0,f1,f2, h0, h1, h2, temp0, temp1, temp2, r, c, u,v,w, ex, ey,ez,Temp,t,f0bc);
...
__global__ void gpu_collide_save(double *f0, double *f1, double *f2, double *h0, double *h1, double *h2, double *temp0, double *temp1, double *temp2,
	double *r, double *c, double *u, double *v, double *w, double *ex, double *ey, double *ez, double *Temperature, double t,double *f0bc)
{
	// useful constants
...
void save_data_tecplot(FILE *fout, double time, double *rho_gpu, double *charge_gpu, double *phi_gpu,
	...
	if (first)
	{
		char str[] = "VARIABLES=\"x\",\"y\",\"z\",\"u\",\"v\",\"w\",\"p\",\"charge\",\"phi\",\"Ex\",\"Ey\",\"Ez\",\"Temperature\"";
	...
	for (unsigned int z = 0; z < NZ; ++z) 
	{
		for (unsigned int y = 0; y < NY; ++y)
		{
			for (unsigned int x = 0; x < NX; ++x)
			{
				fprintf(fout, "%g %g %g %g %g %g %g %g %10.6f %10.6f %10.6f %10.6f %10.6f\n", dx_host*x, dy_host*y, dz_host*z,
...
void save_data_end(FILE *fend, double time, double *rho_gpu, double *charge_gpu, double *phi_gpu,
	...
	for (unsigned int z = 0; z < NZ; ++z)
	{
		for (unsigned int y = 0; y < NY; ++y)
		{
			for (unsigned int x = 0; x < NX; ++x)
			{
				fprintf(fend, "%10.6f %10.6f %10.6f %10.6f %10.6f %10.6f %10.6f %10.6f %10.6f %10.6f %10.6f\n", time,
					ux[scalar_index(x, y, z)], uy[scalar_index(x, y, z)], uz[scalar_index(x, y, z)], rho[scalar_index(x, y, z)], charge[scalar_index(x, y, z)],
					phi[scalar_index(x, y, z)], Ex[scalar_index(x, y, z)], Ey[scalar_index(x, y, z)], Ez[scalar_index(x, y, z)], temp[scalar_index(x, y, z)]);
			}
		}
...
void read_data(double *time, double *rho_gpu, double *charge_gpu, double *phi_gpu,
	...
	for (unsigned int z = 0; z < NZ; ++z)
	{
		for (unsigned int y = 0; y < NY; ++y)
		{
			for (unsigned int x = 0; x < NX; ++x)
			{
				fscanf(fread, "%lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf", time,
					&ux[scalar_index(x, y, z)], &uy[scalar_index(x, y, z)], &uz[scalar_index(x, y, z)], &rho[scalar_index(x, y, z)], &charge[scalar_index(x, y, z)],
					&phi[scalar_index(x, y, z)], &Ex[scalar_index(x, y, z)], &Ey[scalar_index(x, y, z)], &Ez[scalar_index(x, y, z)], &temp[scalar_index(x, y, z)]);
			}
		}
