...
int main(int argc, char* argv[])
{
	...
	if (flag == 1) {
		...
		read_data(&t, rho_gpu, charge_gpu, phi_gpu, ux_gpu, uy_gpu, uz_gpu, Ex_gpu, Ey_gpu, Ez_gpu, T_gpu);
	}
	else {
		printf("Initializing...\n");
		// Zero flow at t=0
		// to initialize rho, charge, phi, ux, uy, Ex, Ey fields.
		initialization(rho_gpu, charge_gpu, phi_gpu, ux_gpu, uy_gpu, uz_gpu, Ex_gpu, Ey_gpu, Ez_gpu, T_gpu);
		t = 0;
	}
       
    // initialise f1,h1 as equilibrium for rho, ux, uy, charge, ex, ey
    init_equilibrium(f0_gpu,f1_gpu,h0_gpu,h1_gpu, temp0_gpu, temp1_gpu, rho_gpu,charge_gpu, ux_gpu,uy_gpu,uz_gpu,Ex_gpu,Ey_gpu,Ez_gpu,T_gpu);

	// open file for writing
	FILE *fout = fopen("data.dat", "wb+");
	save_data_tecplot(fout, t, rho_gpu, charge_gpu, phi_gpu, ux_gpu, uy_gpu, uz_gpu, Ex_gpu, Ey_gpu, Ez_gpu, T_gpu, 1);
	...
	save_data_end(fend, t, rho_gpu, charge_gpu, phi_gpu, ux_gpu, uy_gpu, uz_gpu, Ex_gpu, Ey_gpu, Ez_gpu, T_gpu);
